...
export const makeListDataReducer = (
  ...
  const listReducer = typeof customListReducer === 'function'
    ? customListReducer
    : defaultListReducer

  const paginationReducer = typeof customPaginationReducer === 'function'
    ? customPaginationReducer
...
// RJ List
export default (config = {}) => rj({
  dataReducer: makeListDataReducer(config.pagination),
  proxySelectors: ({ getData }) => makeListSelectors(getData, config.pageSize),
})
