...
export const {
  ...
  dataReducer: () => null,
  state: 'todos.add',
  effect: todo => request.post(`${API_URL}/todos`)
    .send(todo)
    .then(({ body }) => body)
})()
...
export const {
  ...
  multiloadingRj,
  {
    ...
    effect: ({ todo }) => request.put(`${API_URL}/todos/${todo.id}`)
      .send(todo)
      .then(({ body }) => body)
  }
)()
...
export const {
  ...
  reducer: reducer => composeReducers(reducer, (prevState, { type, payload, meta }) => {
    switch (type) {
